...
flags.DEFINE_string("task_name", 'panc', "The name of the task to train.")
...
flags.DEFINE_integer("save_summary_steps", 300,
                     "save_summary_steps.")
...
class PaddingInputExample(object):
    ...
    When  ning eval/predict on the TPU, we need to pad the number of examples
...
class PancProcessor(DataProcessor):
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    # 转成wordpiece，把一些长词又进行了切分，第一句话分词
    tokens_b = None
    if example.text_b:
    ...
    # (a) For sequence pairs:（0代表前一句话，1代表后一句话）
    ...
    input_ids = tokenizer.convert_tokens_to_ids(tokens)  # 把词转换成id，vocab.txt

    # The mask has 1 for real tokens and 0 for padding tokens. Only real
    # tokens are attended to.
    # pad0的地方，self-attetion不需要注意到，所以把补0的地方mask掉，只有input_mask为1的地方是实际的词
    input_mask = [1] * len(input_ids)

    # Zero-pad up to the sequence length.未达最大长度则进行补齐操作
...
def file_based_convert_examples_to_features(
    ...
    """Convert a set of `InputExample`s to a TFRecord file.运行速度快"""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tf_example = tf.train.Example(features=tf.train.Features(
            feature=features))  # tensorflow的数据结构
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    """Creates a classification model.构建模型"""
    ...
    # In the demo, we are doing a simple classification task on the entirebi-direction maximum matching
    # segment.
    #
    # If you want to use the token-level output, use model.get_sequence_output()
    # instead.
    output_layer = model.get_pooled_output()  # shape (batch_size,768)

    hidden_size = output_layer.shape[-1].value

    output_weights = tf.get_variable(
        "output_weights", [num_labels, hidden_size],
        initializer=tf.truncated_normal_initializer(stddev=0.02))  # 全连接层进行分类
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            hook_dict['output_layer'] = output_layer
            a = output_layer.shape
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        result = estimator.predict(input_fn=predict_input_fn, checkpoint_path=FLAGS.output_dir + 'model.ckpt-2500')
